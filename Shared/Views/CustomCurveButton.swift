...
struct CustomCurveButton: Shape {
    func path(in rect: CGRect) -> Path {
        return Path { path in
            ...
            path.addLine(to: CGPoint(x: rect.width + 100, y: rect.height - 76))
            path.addLine(to: CGPoint(x: rect.height - 100, y: rect.width + 50))
            
            let middle = rect.width / 2
            
            path.move(to: CGPoint(x: middle - 100, y: 0))
